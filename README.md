...
Lexical analyzer generation using `Flex` has been implemented (see [lexer.l](https://github.com/RTCupid/ParaCL_Compiler/blob/main/frontend/src/lexer.l)).
...
Token processing functions are defined in the `Lexer` class, which inherits from `yyFlexLexer` (see [lexer.hpp](https://github.com/RTCupid/ParaCL_Compiler/blob/main/frontend/include/parser/lexer.hpp)). They return the corresponding parser token generated by `Bison`. This is done for joint operation of `Bison` and `Flex`.
...
For syntax analysis, the `My_parser` class has been added (see [my_parser.hpp](https://github.com/RTCupid/ParaCL_Compiler/blob/main/frontend/include/parser/my_parser.hpp)). It inherits from `yy::parser`, which is generated using `Bison` (see [parser.y](https://github.com/RTCupid/ParaCL_Compiler/blob/main/frontend/src/parser.y)), and contains the following fields and methods:
...
## Node pool for AST node allocation
`Node_pool` is used to create AST nodes. It allocates objects derived from `Node`, stores them internally, and manages their lifetime automatically.

The ```make<T>(...)``` method constructs a node of the requested type, keeps it in the pool, and returns a raw pointer to the created object. This simplifies AST construction and removes the need for manual memory management.

<details>
<summary>Node_pool class</summary>
...
To collect errors, an `Error_collector` has been implemented (see [error_collector.hpp](https://github.com/RTCupid/ParaCL_Compiler/blob/main/frontend/include/parser/error_collector.hpp)).
...
To simulate program execution, a `Simulator` class has been implemented (see [simulator.hpp](https://github.com/RTCupid/ParaCL_Compiler/blob/main/frontend/include/simulator/simulator.hpp)), which inherits from the abstract `ASTVisitor` class:
...
In the `Simulator` class, virtual functions of `ASTVisitor` are overridden, and a function for evaluating expressions is introduced, which uses a special `ExpressionEvaluator` class (see [expr_evaluator.hpp](https://github.com/RTCupid/ParaCL_Compiler/blob/main/frontend/include/simulator/expr_evaluator.hpp)):
